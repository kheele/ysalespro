...
function generateFallbackOutreach(input: GenerateSalesOutreachInput): GenerateSalesOutreachOutput {
  ...
  const personTitle = input.person.title || input.person.role || 'Leader';
  ...
  const trigger = input.company.recent_news ? `following your recent news on "${input.company.recent_news}"` : `as ${companyName} continues scaling`;
    ...
    messages: [
      {
        ...
        content: `1. Question regarding ${companyName}'s pipeline velocity\n2. Quick idea for ${personName} regarding ${industry} compliance\n3. ${companyName} + YSalesPro: 15-minute operational sync`,
        score: 95,
      ...
      },
      {
        ...
        content: `Hi ${personName}, noticed your impressive leadership as ${personTitle} at ${companyName}. Would love to connect and share insights on what we're seeing in the ${industry} space!`,
        score: 95,
  ...
  async (input) => {
    try {
      const prompt = `You are an elite B2B Sales Outreach Strategist and Copywriter.
        Generate a complete, hyper-personalized sales outreach sequence in valid JSON format matching the schema.
  ...
  async (input) => {
    ...
    return {
      pain_points: [
        ...
        `Compliance, governance, and audit readiness in ${input.industry}`,
...
export async function generateSingleMessageFlow(req: AIMessageRequest): Promise<AIMessageResponse> {
  ...
  try {
    const prompt = `You are an elite B2B sales copywriter and strategist.
      ...
      Provide a compelling subject (if email), message body content, and array of 2-3 key hooks used.`;
