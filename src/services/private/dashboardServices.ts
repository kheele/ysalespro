...
export async function getKPIsActionByToken(token: string): Promise<DashboardKPIs> {
  try {
    ...
    const cold = leadsList.filter(l => (l.stage || 'Cold') === 'Cold' || l.stage === 'Contacted').length;
    const warm = leadsList.filter(l => l.stage === 'Warm').length;
    const hot = leadsList.filter(l => l.stage === 'Hot' || l.stage === 'Customer').length;
